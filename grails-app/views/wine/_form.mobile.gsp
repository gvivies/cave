<%@ page import="mycompany.Wine" %>
		...
		<div class="ui-grid-a">
			...
			<div class="ui-block-b ${hasErrors(bean: wineInstance, field: 'region', 'error')} required">
				...
				<g:select data-native-menu="false"  id="region" name="region.id" from="${mycompany.Region.list()}" optionKey="id" required="" value="${wineInstance?.region?.id}" optionValue="name"  class="many-to-one"/>
			</div>
		</div>	
